...
void CMemory::SetupHooks( ) {
    CreateHook( &PrintDeclare, XOR( "" ) );

}

template <typename hook_type>
void CMemory::CreateHook( hook_type* Hook, const std::string& Signature ) {
...
template <typename hook_type>
void CMemory::CreateHook( hook_type* Hook, const uintptr_t& Offset ) {
    *Hook = reinterpret_cast< hook_type >(
        Rebase( Offset )
    );
}

// @Credits ( @MarkHC, @Hinnie, @DucaRii ): https://github.com/reversed2603/gamesense_legacy/blob/main/pandora-final/SDK/Utils/PatternScan.cpp#L8
...
INLINE std::uintptr_t CMemory::Scan( const std::string& Signature ) {
...
INLINE std::uintptr_t CMemory::Rebase( std::uintptr_t Address ) {
