...
namespace hacks {
	...
	__forceinline void c_eng_pred::local_data_t::init( const valve::user_cmd_t& user_cmd ) {
		...
		if ( g_ctx->weapon( ) ) {
			g_eng_pred->recoil_index( ) = g_ctx->weapon( )->recoil_index( );
			g_eng_pred->accuracy_penalty( ) = g_ctx->weapon( )->accuracy_penalty( );
		}
		...
		m_net_vars.m_abs_origin = valve::g_local_player->get_abs_origin( );
	...
	__forceinline void c_eng_pred::local_data_t::save( const int cmd_number ) {
		...
		m_pred_net_vars.m_abs_origin = valve::g_local_player->get_abs_origin( );
