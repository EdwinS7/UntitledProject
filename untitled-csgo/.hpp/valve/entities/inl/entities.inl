...
namespace valve {
	__forceinline c_entity* c_client_unknown::get_entity( ) {
		using fn_t = c_entity * ( __thiscall* )( decltype( this ) );

		return ( *reinterpret_cast< fn_t** >( this ) )[ 7u ]( this );
	}

	__forceinline c_client_unknown* c_renderable::get_client_unknown( ) {
		using fn_t = c_client_unknown * ( __thiscall* )( decltype( this ) );
    ...
    __forceinline const vec3_t& c_entity::get_abs_origin( ) {
        using fn_t = const vec3_t&( __thiscall* )( decltype( this ) );

        return ( *reinterpret_cast< fn_t** >( this ) )[ 10u ]( this );
    }
	...
	__forceinline void c_entity::set_abs_angles( const qangle_t& abs_angles ) {
		...
		return reinterpret_cast< fn_t >(
			g_ctx->addresses( ).m_set_abs_angles
		)( this, abs_angles );
	}
	...
	__forceinline bool c_entity::breakable( ) {
		...
		if ( reinterpret_cast< fn_t >( g_ctx->addresses( ).m_breakable )( this ) )
			return true;
	...
	__forceinline bool c_entity::friendly( ) {
		return team( ) == valve::g_local_player->team( );
	}
	...
	__forceinline void c_entity::set_collision_bounds(
		...
		auto& obb_height = *reinterpret_cast< float* >(
			reinterpret_cast< std::uintptr_t >( this ) + 0x9920u
		);

		auto& obb_change_time = *reinterpret_cast< float* >(
			reinterpret_cast< std::uintptr_t >( this ) + 0x9924u
		);

		const auto backup_obb_height = obb_height;
	...
	__forceinline weapon_data_t* c_weapon::wpn_data( ) {
		...
		const auto wpn_sys = g_ctx->addresses( ).m_weapon_system;
	...
	__forceinline bool c_player::has_c4( ) {
		...
		return reinterpret_cast< fn_t >( g_ctx->addresses( ).m_has_c4 )( this );
	}
