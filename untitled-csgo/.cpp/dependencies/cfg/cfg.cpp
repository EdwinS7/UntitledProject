...
namespace sdk {
	void c_cfg::save( const std::string_view name ) const {
		const auto path = std::filesystem::path{ ENCSTR( SDK_CFG_ID_OBJECT ) } /= name;

		/* create directory if its not exists */
		std::filesystem::create_directory( ENCSTR( SDK_CFG_ID_OBJECT ) );
	...
	void c_cfg::load( const std::string_view name ) {
		...
		const auto object = json.find( ENCSTR( SDK_CFG_ID_OBJECT ) );
		if ( object == json.end( ) )
			return;
