...
namespace hacks {
	...
	void c_visuals::on_paint( ) {
		...
		if ( g_movement->cfg( ).m_auto_peek_key
			&& HIWORD( GetKeyState( g_movement->cfg( ).m_auto_peek_key ) ) ) {
			
			vec2_t screen_pos{};
			if ( math::to_screen( g_movement->auto_peek_start_pos( ), ImGui::GetIO( ).DisplaySize, valve::g_engine->w2s_matrix( ), screen_pos ) ) {
				add_filled_circle( screen_pos, 16, 16, ImColor( 240, 240, 240 ) );
			}
		}
		...
		if ( m_cfg->m_other.m_indicators & 6 ) {
			...
			if ( cfg.m_force_baim_key
				&& HIWORD( GetKeyState( cfg.m_force_baim_key ) ) ) {
				add_text(
		...
		**reinterpret_cast< bool** >(
		...
		if ( m_cfg->m_player.m_sounds )
			for ( auto i = m_player_sounds.begin( ); i != m_player_sounds.end( ); i = m_player_sounds.erase( i ) )
				add_beam_ring( *i, m_cfg->m_player.m_sounds_clr );
		else
