...
namespace hacks {
	void c_eng_pred::prepare( ) {
		if ( valve::g_client_state->m_signon_state == 6 && valve::g_client_state->m_delta_tick >= 0 ) {
			...
			if ( g_ctx->weapon( ) )
				if ( g_ctx->weapon( )->item_index( ) == valve::e_item_index::revolver
					&& g_ctx->weapon( )->postpone_fire_ready_time( ) == std::numeric_limits< float >::max( ) )
					g_ctx->weapon( )->postpone_fire_ready_time( ) = m_postpone_fire_ready_time;
		}
	...
	void c_eng_pred::predict( valve::user_cmd_t* const user_cmd ) {
		...
		g_ctx->addresses( ).m_pred_player = valve::g_local_player;
		*g_ctx->addresses( ).m_pred_seed = user_cmd->m_random_seed;
		...
		if ( g_ctx->weapon( ) ) {
			g_ctx->weapon( )->update_inaccuracy( );

			m_inaccuracy = g_ctx->weapon( )->inaccuracy( );
			m_spread = g_ctx->weapon( )->spread( );
			...
			if ( valve::g_local_player->flags( ) & valve::e_ent_flags::ducking )
				m_min_inaccuracy = v11 ? g_ctx->wpn_data( )->m_inaccuracy_crouch_alt : g_ctx->wpn_data( )->m_inaccuracy_crouch;
			else
				m_min_inaccuracy = v11 ? g_ctx->wpn_data( )->m_inaccuracy_stand_alt : g_ctx->wpn_data( )->m_inaccuracy_stand;
		}
		else
