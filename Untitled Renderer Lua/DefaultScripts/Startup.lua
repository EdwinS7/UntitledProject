...
function Paint()
    ...
    -- Logs (Top-Left)
    local ConsoleLogs = Client.GetLogs(Enums.LogLevel.All)
    local String = table.concat(ConsoleLogs, "\n")

    Renderer.Text(Renderer.GetDefaultFont(), String, Vector2.new(5, 5), Color.new(0, 0, 0, 255))
end

Connect("OnInputUpdate", F11Fullscreen)
