...
void c_offsets::init( ) {
	...
	create_hook( &g_hooks->print, "55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC 20 A1 ?? ?? ?? ?? 33 C5 89 45 F0 50 8D 45 F4 64 A3 00 00 00 00 8B" );

	// @ida xref: [Watchdog or LuauWatchdog], after second jnz its the sub function.
	create_hook( &g_hooks->get_scheduler, "55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 51 56 A1 ?? ?? ?? ?? 33 C5 50 8D 45 F4 64 A3 00 00 00 00 E8 ?? ?? ?? ?? 84" );

	// @ida xref: "defer", the function itself.
	create_hook( &g_hooks->task_defer, "55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC 30 A1 ?? ?? ?? ?? 33 C5 89 45 EC 53 56 57 50 8D 45 F4 64 A3 00 00 00 00 8B 7D 08 C7 45 E8" );

	// @ida xref: "spawn", the function itself.
	create_hook( &g_hooks->task_spawn, "55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC 24 A1 ?? ?? ?? ?? 33 C5 89 45 EC 56 57 50 8D 45 F4 64 A3 00 00 00 00 8B 7D 08 C7" );

	// @ida xref: "debug" second sub routine upwards ( use match case ), its right under that.
	create_hook( &g_hooks->get_state, "55 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 83 EC 2C A1 ?? ?? ?? ?? 33 C5 89 45 F0 53 56 57 50 8D 45 F4 64 A3 00 00 00 00 8B F9 8B 45 0C" );

	// @ida xref: "oldResult, moduleRef  = ..." last sub function called.
	create_hook( &g_hooks->lua_vm_load, "53 8B DC 83 EC 08 83 E4 F8 83 C4 04 55 8B 6B 04 89 6C 24 04 8B EC 6A FF 68 ?? ?? ?? ?? 64 A1 00 00 00 00 50 51 53 81 EC D0 07" );

	// @ida xref: "The metatable is locked" located in the loc_xxxxxxx above. its the sub function being used.
	create_hook( &g_hooks->pushkclosure, "E8 ?? ?? ?? ?? 83 C4 0C 8B 50 14 8D 48 28" );
