...
std::uintptr_t cMemory::Scan( const std::string& Signature ) {
    auto image = reinterpret_cast< std::uintptr_t >( GetModuleHandleA( TARGET_NAME ) );
    if ( !image )
        return 0u;
...
std::uintptr_t cMemory::Rebase( std::uintptr_t Address ) {
	return Address - PE + BaseAddress;
}